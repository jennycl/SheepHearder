...
function draw(){
  ...
  //if(gameState == 1){
    ...
    // check collision between sheep - randomly moving 
    
    for (var j = 0; j < sheeps.length ;j++){
      if (sheepToPenCollision(sheeps[j])){
        sheeps.splice(j, 1);
      }
    }
...
function sheepToPenCollision(sheep){
  if (sheep.x > canvasWidth - canvasWidth/10 && (sheep.y < canvasHeight/2 + canvasHeight/4) && (sheep.y > canvasHeight/2 - canvasHeight/4)){
    sheepSound.play();
    return true;
  } 
  else{
